  ...
  defmacro __using__(opts) do
    quote(location: :keep) do
      defmodule Api do
        ...
        def fetch(method, api_method, body, query) when is_list(query),
          do: fetch(method: method, url: @api_path <> api_method, body: body, query: query)

        def fetch(method, api_method, query) when is_list(query),
          do: fetch(method: method, url: @api_path <> api_method, query: query)

        def fetch(method, api_method, body),
          do: fetch(method: method, url: @api_path <> api_method, body: body)
        ...
        def fetch(args) do
          args =
            cond do
              sending_file?(args[:body]) ->
                Keyword.put(args, :body, multipart_body(args[:body]))

              sending_file?(args[:query]) ->
                Keyword.put(args, :body, multipart_body(args[:query])) |> Keyword.delete(:query)
          ...
          with {:ok, %Tesla.Env{status: status, body: body}} <- Tesla.request(client(), args) do
            if status in 200..299 do
              ...
              Logger.warning(
                "Telegram Bot API error requesting #{fetch_args_info(args)} responded HTTP #{status}: '#{comment}'\nResponse body: #{inspect(body)}"
              )
            ...
            {:error, error} ->
              Logger.warning(
                "Telegram Bot API connection error #{fetch_args_info(args)}: #{inspect(error)}"
              )
        ...
        def fetch_args_info(args) do
          method =
            if not is_nil(args[:method]), do: args[:method] |> to_string |> String.upcase()

          token = unquote(opts[:token] || "")

          url =
            if not is_nil(args[:url]),
              do: args[:url] |> String.replace(token, "<token>")
        ...
        defp multipart_body(body_or_query) do
          Enum.reduce(body_or_query, Tesla.Multipart.new(), fn
            {name, {:file, file}}, multipart ->
              ...
              Tesla.Multipart.add_file(multipart, file,
                name: to_string(name),
            ...
            {name, {:file_content, file_content, filename}}, multipart ->
              ...
              Tesla.Multipart.add_file_content(multipart, file_content, filename,
                name: to_string(name),
                headers: [{"content-length", to_string(size)}]
              )
